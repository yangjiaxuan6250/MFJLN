...
from common.Dense_Haze import Dense_Dataset
from common.GT_RAIN import GTRAIN_Dataset
from common.SPA_DATA import SPA_Dataset
from common.UAV_DATA import UAV_Dataset
from common.Internet_Data import Intertnet_Dataset
# import math
...
import h5py
from common.Rain100H import Rain100H
from common.RainDS_DATA import RainDS_Dataset
...
class derainSession():
    ...
    def get_dataloader(self, dataset_name, distributed):
        ...
        if dataset_name == "DDN" or dataset_name == "rain12600":
            ...
            dataset = SPA_Dataset(os.path.join(self.args.data_dir, "SPA/train"), self.patch_size, eval=False)
        elif dataset_name == 'Dense':
            dataset = Dense_Dataset(os.path.join(self.args.data_dir, "indoor/train"), self.patch_size, eval=False)
        elif dataset_name == 'UAV':
            dataset = UAV_Dataset(os.path.join(self.args.data_dir, "UAV-Rain1k/train"), self.patch_size, eval=False)
        elif dataset_name == 'LHP':
            dataset = GTRAIN_Dataset(os.path.join(self.args.data_dir, "LHP/train"), self.patch_size, eval=False)
        elif dataset_name == 'RainDS':
            dataset = RainDS_Dataset(os.path.join(self.args.data_dir, "RainDS/RainDS_syn"), self.patch_size, eval=False)
    ...
    def get_eval_dataloader(self, dataset_name, distributed):
        ...
        if dataset_name in ['Rain200H', 'Rain200L']:
            ...
            dataset = RainDS_Dataset(os.path.join(self.args.data_dir, "RainDS/RainDS_syn"), self.patch_size, eval=True, dataset_type='rd')
        elif dataset_name == 'UAV':
            dataset = UAV_Dataset(os.path.join(self.args.data_dir, "UAV-Rain1k/test"), self.patch_size, eval=True)
        elif dataset_name == 'Rain100H':
            dataset = Rain100H(os.path.join(self.args.data_dir, "Rain100H"), self.patch_size, eval=True)
        elif dataset_name == 'Rain100L':
            dataset = Rain100H(os.path.join(self.args.data_dir, "Rain100L"), self.patch_size, eval=True)
        elif dataset_name == 'SPA':
            dataset = SPA_Dataset(os.path.join(self.args.data_dir, "SPA/test"), self.patch_size, eval=True)
        elif dataset_name == 'Dense':
            dataset = Dense_Dataset(os.path.join(self.args.data_dir, "indoor/test"), self.patch_size, eval=True)
        elif dataset_name == 'LHP':
            dataset = GTRAIN_Dataset(os.path.join(self.args.data_dir, "LHP/test"), self.patch_size, eval=True)
        elif dataset_name == 'Internet':
            dataset = Intertnet_Dataset(os.path.join(self.args.data_dir, "BDD350"), self.patch_size, eval=True)
...
class TrainValDataset(Dataset):
    ...
    def augment(self, imgA, imgB, hflip=True, rot=True):
