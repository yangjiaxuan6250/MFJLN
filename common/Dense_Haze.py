...
def augment(imgA, imgB, hflip=True, rot=True):
    ...
    if r:
        imgA = np.rot90(imgA).copy()
        imgB = np.rot90(imgB).copy()
    return imgA, imgB
...
class Dense_Dataset(udata.Dataset):
    def __init__(self, data_path, patch_size, eval):
        super(Dense_Dataset, self).__init__()
        # train
        print('process training data')
        self.input_path = os.path.join(data_path, 'haze')
        self.target_path = os.path.join(data_path, 'gt')
        self.patch_size = patch_size
        self.imgs = os.listdir(self.input_path)
        self.gt = os.listdir(self.target_path)
        self.eval = eval
        self.offset_list = []
        self.index_list = []
    ...
    def __getitem__(self, idx):
        ...
        target_file = self.gt[idx % self.num]
...
#
#     data_path = "D:\\dataset\\spa\\train"
#     dataset = SPA_Dataset(data_path, (128, 128), False)
#     print(dataset[0])
