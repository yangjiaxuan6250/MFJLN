...
class EpochBasedRunner(BaseRunner):
    ...
    def val(self, data_loader, **kwargs):
        ...
        for i, (data_batch, batch_indices) in enumerate(zip(self.data_loader, loader_indices)):
            self._inner_iter = i
            self.call_hook('before_val_iter')
            self.run_iter(data_batch, train_mode=False, idx=i, batch_indices=batch_indices,
                          img_range=self.opt_cfg['img_range'],
                          save_fmt=self.opt_cfg['save_fmt'], filename=data_batch.get('filename', None), save_dir=self.save_dir)
            self.call_hook('after_val_iter')


        self.call_hook('after_val_epoch')
        if self.opt_cfg['eval'] or not self.train_flag:
            print_log("find it not in training process, stop it in runner.val", logger=self.logger)
    ...
    def run(self, data_loaders, workflow, max_epochs=None, **kwargs):
        ...
        self.train_flag = any('train' in mode for mode, _ in workflow)
        ...
        while self.epoch < self._max_epochs:
            for i, flow in enumerate(workflow):
                ...
                for epoch in range(epochs):
                    if mode == 'train' and epoch >= self._max_epochs:
                        break
                    epoch_runner(data_loaders[mode], **kwargs)
            # if self.earlyStop:
            #     print_log("model's train has diverged, python will stop training", logger=self.logger)
            #     break
