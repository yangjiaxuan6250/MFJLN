...
def trainer(cfg, logger, build_model,
    ...
    if distributed:
        ...
        if not hasattr(model, 'train'):  # 任务分配器是否注册为模型，注册了就会有'train'
            if isinstance(model.model, dict):  # 实际运行的模型可以有多个，通过字典区分
                ...
                # dist.init_process_group(backend='gloo', rank=cfg.local_rank)
                # print("!!!!!!!!!!!!!!")
                # model.model = MMDistributedDataParallel(model.model, device_ids=cfg.gpu_ids)
        else:
    ...
    if os.path.isfile(cfg.config):
        ...
        runner.register_checkpoint_hook(
            dict(type='ModelCheckpoint', indicator='loss', save_top_k=cfg.save_top_k, print_freq=cfg.save_times, warmup_epoch=cfg.warmup_epoch))
    ...
    for flow in cfg.workflow:
        ...
        if 'val' in mode:
            ...
            if mode != 'simple_val':
                runner.register_hook(
                    eval_hook(eval_loader, save_best= 'auto', **eval_cfg), priority='LOW')
